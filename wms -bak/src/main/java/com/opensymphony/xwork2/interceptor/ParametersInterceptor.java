...
public class ParametersInterceptor extends MethodFilterInterceptor {
	...
	protected void notifyDeveloperParameterException(Object action, String property, String message) {
		String developerNotification = LocalizedTextUtil.findText(ParametersInterceptor.class, "devmode.notification",
				...
				new Object[] { "Unexpected Exception caught setting '" + property + "' on '" + action.getClass() + ": "
						+ message });
	...
	protected boolean isAcceptableParameter(String name, Object action) {
		ParameterNameAware parameterNameAware = (action instanceof ParameterNameAware) ? (ParameterNameAware) action
				: null;
	...
	protected boolean isWithinLengthLimit(String name) {
		...
		if (!matchLength) {
			notifyDeveloper("Parameter [#0] is too long, allowed length is [#1]", name,
					String.valueOf(paramNameMaxLength));
		}
		return matchLength;
	}
